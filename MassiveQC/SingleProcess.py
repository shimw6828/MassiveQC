...
# sys.path.insert(0, "/home/mwshi/github/")
...
def process(SRR):
    ...
    if summary_file.exists():
        logger.info(f"{SRR} fastq file has been checked, {summary_file}")
    else:
        check_fq(SRR, download_path, QC_dir, feature_path)
    logger.info(f"Complete check {SRR} fastq file")

    # fast_screen
    fast_screen_output = feature_path / "fastq_screen" / f"{SRR}.parquet"
    if fast_screen_output.exists():
        logger.info(f"{SRR} fast_screen step has been done")
    else:
    ...
    (feature_path / "DoneSample" / f"{SRR}").touch()
    return SRR
